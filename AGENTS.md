...
Issues and PRDs are tracked in GitHub Issues for `Libai-88/RUI`. See `docs/agents/issue-tracker.md`.

### Triage labels

The repository uses the default `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, and `wontfix` labels. See `docs/agents/triage-labels.md`.

### Domain docs

The repository uses a single-context layout with root `CONTEXT.md` and `docs/adr/`. See `docs/agents/domain.md`.
